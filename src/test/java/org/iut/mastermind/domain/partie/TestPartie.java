...
@DisplayName("Test si une partie:")
public class TestPartie {
    ...
    @Test
    @DisplayName("retourne l'évaluation du mot proposé")
    void doitRenvoyerResultatPourLaProposition() {
        givenPartieEnregistree(Partie.nouvelle(JOUEUR, MOT_CORRECT));
    ...
    void doitTerminerLaPartieQuandNombreEssaisDepasse(){
        int nbEssaiMax = 5;
        givenPartieEnregistree(Partie.reprendre(JOUEUR, MOT_CORRECT, nbEssaiMax-1));
    ...
    void doitLeverErreurQuandJeuTermine(){
        var partie = Partie.nouvelle(JOUEUR, MOT_CORRECT);
        partie.done();
